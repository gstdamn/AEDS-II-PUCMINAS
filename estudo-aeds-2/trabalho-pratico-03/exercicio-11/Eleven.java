...
class ListaDupla
{
    ...
    void quickSort(Celula esq , Celula dir){
        ...
        while(i != j){
            while(comparar(i.restaurante, pivo) <= 0 && i != j)
                i = i.prox;
            while(comparar(j.restaurante, pivo) >= 0 && i != j)
                j = j.ant;
            ...
            if(i != j){
                ...
                j.restaurante = tmp;
                Eleven.movimentacoes += 3;
            }
        }


        if(comparar(i.restaurante, pivo) > 0){ // este if serve para corrigir a posicao do pivo depois de ocorrer a troca dos restaurantes
            Restaurante tmp = i.restaurante;
            i.restaurante = dir.restaurante;
            dir.restaurante = tmp;
            Eleven.movimentacoes +=3;
        }

       if(i != esq && i.ant != null && i.ant != esq.ant) 
        quickSort(esq, i.ant);
       if(i != dir && i.prox != null)
...
public class Eleven {
  ...
  public static void main(String[] args) throws Exception{
    ...
    FileWriter log = new FileWriter("905087_quicksort_flexivel.txt");
