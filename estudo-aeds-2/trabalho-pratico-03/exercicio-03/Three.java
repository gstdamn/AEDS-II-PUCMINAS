...
//construtor da classe restaurante
  ...
  void swap(Restaurante[]  r, int i, int j){
    Restaurante tmp = r[i];
    r[i] = r[j];
    r[j] = tmp;
    Three.movimentacoes += 3;
  }
    int particao(Restaurante[] r, int baixo, int alto){
    Restaurante pivo = r[alto];

    int i = baixo - 1;

    for(int j = baixo; j < alto; j++){
        if(comparar(r[j], pivo) <= 0){
            i++;
            swap(r, i, j);
            Three.comparacoes += 2;
          }   
  ...
  int comparar(Restaurante r1, Restaurante r2 ){
    ...
    if(r1.getAvaliacao() < r2.getAvaliacao())
        resp = -1;
    else if(r1.getAvaliacao() > r2.getAvaliacao())
        resp = 1;
    else
        resp = r1.getNome().compareTo(r2.getNome());

    return resp;
  }  

  void quickSort(Restaurante[] r, int baixo, int alto, int k){  
      
      int i = baixo, j = alto;
      Restaurante pivo = r[(baixo + alto) / 2];

      while(i <= j){
        while(comparar(r[i], pivo) < 0)
            i++;
        while(comparar(r[j], pivo) > 0)
            j--;
      ...
      if(baixo < j)
          quickSort(r,baixo, j, k);
      if( i < alto && i < k)
          quickSort(r,i, alto, k);
    }
...
public class Three {
  ...
  public static void main(String[] args) throws Exception{
    ...
    colecao.quickSort(entradas, 0 , tamanho - 1, k);
    colecao.mostrar(entradas, tamanho);
    double fim = new Date().getTime();
    double tempoExecucao = (fim - inicio) / 1000.0;

    FileWriter log = new FileWriter("905087_quicksort_parcial.txt");
